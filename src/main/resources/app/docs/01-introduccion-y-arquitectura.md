...
1. **Registro:** Al arrancar, nuestro Servidor MCP le dice al Host: _"Oye, tengo una herramienta llamada `hello-world` y, por cierto, tiene una interfaz gráfica asociada. Su HTML está en esta ruta."_
2. **Ejecución:** El usuario (o el LLM) en el Host decide usar la herramienta `hello-world`.
...
4. **Comunicación Bidireccional:** Nuestra UI en React utiliza un SDK (`@modelcontextprotocol/ext-apps/react`) para "hablar" con el Host. La UI le pide al Host: _"Por favor, dile al Servidor que ejecute la lógica de `hello-world` y devuélveme el resultado para que yo lo pinte bonito en pantalla"_.
